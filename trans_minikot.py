...
# ============================================
num_row = 1
scopes = {'main': {'code': [], 'vars': {}, 'labels': {}, 'params_count': 0}}
current_scope = 'main'
global_vars = {}
func_signatures = {}
labelCounter = 0
...
def parse_token(expected_lex, expected_tok):
    ...
    else: print(f"Error line {line}: Exp {expected_lex}, Got {lex}"); sys.exit(1)

def gen(lex, tok):
    if generate_on: scopes[current_scope]['code'].append((lex, tok))
...
def setLabel(label):
    gen(label, 'label'); gen(':', 'colon'); scopes[current_scope]['labels'][label] = 0 
...
def parse_statement():
    ...
    elif tok == 'id': parse_assign()
    elif tok == 'bracket_op' and lex == '{': parse_block()
...
def parse_assign():
    ...
    if next_l == '(': 
         parse_token('(', 'bracket_op'); _parse_call_args(); parse_token(')', 'bracket_op'); parse_token(';', 'punct')
         gen(name, 'CALL'); return

    parse_token('=', 'assign_op'); gen(name, 'l-val'); expr_type = parse_expression()
    
    var_type = scopes[current_scope]['vars'].get(name) or global_vars.get(name, 'int')
...
def parse_fun_decl():
    ...
    scopes[func_name] = {'code': [], 'vars': {}, 'labels': {}, 'params_count': 0}
    param_count = 0; _, l, _ = get_symb()
    if l != ')':
        while True:
             _, p_name, _ = get_symb(); parse_token(None, 'id')
             parse_token(':', 'punct'); _, p_type, _ = get_symb(); parse_token(None, 'keyword')
             psm_type = 'int'
    ...
    _, ret_lex, _ = get_symb(); parse_token(None, 'keyword') 
    ret_type = 'int'
    ...
    parse_block()
    if not scopes[func_name]['code'] or scopes[func_name]['code'][-1][0] != 'RET': gen('RET', 'RET')
    current_scope = 'main'

def parse_return():
    parse_token('return', 'keyword'); parse_expression(); gen('RET', 'RET'); parse_token(';', 'punct')
...
def parse_if():
    ...
    parse_statement(); m2 = createLabel(); gen(m2, 'label'); gen('JMP', 'jump'); setLabel(m1) 
    _, lex, _ = get_symb()
...
def parse_read_call():
    _, lex, _ = get_symb(); parse_token(lex, 'keyword'); parse_token('(', 'bracket_op'); parse_token(')', 'bracket_op')
    gen('INP', 'inp_op')
...
def parse_relation():
    ...
    if tok == 'rel_op':
        ...
        if l_type == 'int' and r_type == 'float': gen('SWAP', 'stack_op'); gen('i2f', 'conv'); gen('SWAP', 'stack_op')
...
def parse_power():
    ...
    while True:
        _, lex, tok = get_symb()
        if tok == 'power_op':
            parse_token('^', 'power_op'); r_type = parse_power() 
            if l_type == 'int': gen('SWAP', 'stack_op'); gen('i2f', 'conv'); gen('SWAP', 'stack_op')
            if r_type == 'int': gen('i2f', 'conv')
            gen('^', 'pow_op'); return 'float'
        else: break
    return l_type

# === МОДИФІКОВАНА ФУНКЦІЯ parse_unary (Трюк з -1) ===
def parse_unary():
    _, lex, tok = get_symb()
    if lex == '-' and tok == 'add_op':
        parse_token('-', 'add_op')
        
        # 1. Рекурсивно парсимо вираз після мінуса
        t = parse_unary()
        
        # 2. Генеруємо множення на -1
        if t == 'float':
            gen('-1.0', 'float')
        else:
            gen('-1', 'int')
            
        gen('*', 'math_op') # Множимо: x * -1
        return t
...
def parse_primary():
    ...
    if tok == 'int_literal': parse_token(lex, tok); gen(lex, 'int'); return 'int'
        ...
        parse_token(lex, 'id'); name = lex; _, next_lex, _ = get_symb()
        if next_lex == '(':
            parse_token('(', 'bracket_op'); _parse_call_args(); parse_token(')', 'bracket_op')
            gen(name, 'CALL'); return func_signatures.get(name, ('int', 0))[0]
        else:
            gen(lex, 'r-val') # ОБОВ'ЯЗКОВО ГЕНЕРУЄМО ЗНАЧЕННЯ (fix l-val error)
